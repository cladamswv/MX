...
func _update_stuck_guard(delta: float) -> void:
    # Failsafe only: if a rider is applying throttle with meaningful speed but
    # physically makes almost no forward progress, lift the bike clear instead
    # of letting the race deadlock. The continuous 3.0 grid should make this rare.
    if airborne or crashed or race_finished or not race_active:
        _stuck_timer = 0.0
        _last_progress_x = global_position.x
        return

    var progress: float = global_position.x - _last_progress_x
    var trying_to_move: bool = _throttle_value > 0.55 and speed > 5.0
    if trying_to_move and progress < 0.035:
        _stuck_timer += delta
    else:
        _stuck_timer = maxf(0.0, _stuck_timer - delta * 2.5)

    if _stuck_timer > 0.85:
        global_position.x += 1.15
        global_position.y += 0.65
        global_position.z = side_depth_z
        velocity = Vector3(maxf(speed, 9.0), 1.6, 0.0)
        speed = maxf(speed, 9.0)
        _stuck_timer = 0.0
