...
func _update_engine_mix(delta: float) -> void:
    ...
    _set_player_mix(_engine_idle_player, idle_gain, -12.0 + rpm * 2.5, 0.84 + rpm * 0.18, delta)
    _set_player_mix(_engine_mid_player, mid_gain, -12.5 + speed_ratio * 3.0, 0.84 + rpm * 0.33, delta)
    _set_player_mix(_engine_high_player, high_gain, -14.5 + speed_ratio * 4.0, 0.88 + rpm * 0.42, delta)
...
func _update_ai_engine_mix(delta: float) -> void:
    ...
    for race_bike: DirtlineBike in racers:
        if race_bike == null or not is_instance_valid(race_bike) or race_bike == player_bike:
            continue
        if not race_bike.race_finished and player_bike != null and is_instance_valid(player_bike) and race_bike.global_position.distance_to(player_bike.global_position) < 24.0:
            active_count += 1
    ...
    var target_db: float = -26.0 + ratio * 4.0
...
func apply_settings() -> void:
    var profile: Node = get_tree().get_first_node_in_group("profile")
    if profile == null:
        return
    _set_bus_volume("Music", profile.music_volume)
    _set_bus_volume("Engines", profile.engine_volume)
    _set_bus_volume("SFX", profile.sfx_volume)
