...
func _build_top_hud() -> void:
    # Gameplay HUD deliberately avoids texture logos. The previous 512x512
    # logo asset could force its texture minimum size on some Android layouts
    # and cover nearly half the race view. Branding is now compact text only.
    var brand: Label = _make_label("DIRTLINE MX", Vector2(18, 10), 18, Color(0.98, 0.985, 0.99))
    brand.name = "GameplayBrand"
    brand.size = Vector2(190, 24)
    brand.add_theme_color_override("font_shadow_color", Color(0, 0, 0, 0.78))
    brand.add_theme_constant_override("shadow_offset_x", 2)
    brand.add_theme_constant_override("shadow_offset_y", 2)
    _gameplay_root.add_child(brand)

    var telemetry: Panel = Panel.new()
    telemetry.name = "TelemetryPanel"
    telemetry.position = Vector2(18, 38)
    telemetry.size = Vector2(410, 68)
    telemetry.add_theme_stylebox_override("panel", panel_style)
    _gameplay_root.add_child(telemetry)

    speed_label = _make_label("000", Vector2(14, -2), 38, Color(0.98, 0.985, 0.99))
    speed_label.size = Vector2(102, 48)
    telemetry.add_child(speed_label)
    telemetry.add_child(_make_label("KM/H", Vector2(17, 43), 10, Color(0.63, 0.66, 0.69)))

    place_label = _make_label("1/7", Vector2(126, 7), 22, Color(1.0, 0.30, 0.04))
    place_label.size = Vector2(72, 30)
    telemetry.add_child(place_label)
    telemetry.add_child(_make_label("POSITION", Vector2(127, 41), 9, Color(0.63, 0.66, 0.69)))

    time_label = _make_label("0:00.00", Vector2(228, 8), 21, Color(0.98, 0.985, 0.99))
    time_label.size = Vector2(160, 30)
    telemetry.add_child(time_label)
    telemetry.add_child(_make_label("RACE TIME", Vector2(230, 41), 9, Color(0.63, 0.66, 0.69)))

    course_label = _make_label("PINE RIDGE  //  FOREST NATIONAL", Vector2(448, 14), 14, Color(0.90, 0.91, 0.92))
    course_label.size = Vector2(220, 22)
    course_label.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
    _gameplay_root.add_child(course_label)
    mode_badge = _make_label("ARCADE RACE", Vector2(448, 39), 11, Color(1.0, 0.31, 0.05))
    mode_badge.size = Vector2(170, 20)
    _gameplay_root.add_child(mode_badge)

    var status: Panel = Panel.new()
    status.name = "RaceStatusPanel"
    status.anchor_left = 1.0
    status.anchor_right = 1.0
    status.anchor_top = 0.0
    status.anchor_bottom = 0.0
    status.offset_left = -594.0
    status.offset_right = -76.0
    status.offset_top = 14.0
    status.offset_bottom = 82.0
    status.add_theme_stylebox_override("panel", panel_style)
    _gameplay_root.add_child(status)

    heat_bar = _make_bar(Vector2(16, 14), Vector2(146, 10))
    status.add_child(heat_bar)
    status.add_child(_make_label("HEAT", Vector2(16, 30), 9, Color(0.65, 0.68, 0.71)))
    boost_bar = _make_bar(Vector2(182, 14), Vector2(146, 10))
    boost_bar.value = 100
    status.add_child(boost_bar)
    status.add_child(_make_label("BOOST", Vector2(182, 30), 9, Color(0.65, 0.68, 0.71)))
    progress_bar = _make_bar(Vector2(348, 14), Vector2(154, 10))
    status.add_child(progress_bar)
    status.add_child(_make_label("PROGRESS", Vector2(348, 30), 9, Color(0.65, 0.68, 0.71)))
    lane_label = _make_label("LINE 3", Vector2(348, 47), 11, Color(1.0, 0.31, 0.05))
    lane_label.size = Vector2(154, 18)
    lane_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
    status.add_child(lane_label)

    var pause: TextureButton = TextureButton.new()
    ...
    pause.offset_left = -64.0
    pause.offset_right = -14.0
    pause.offset_top = 18.0
    pause.offset_bottom = 68.0
...
func _build_controls() -> void:
    ...
    # Compact bottom controls leave the middle 70% of the screen unobstructed.
    ...
    left_pad.offset_left = 14.0
    left_pad.offset_right = 384.0
    left_pad.offset_top = -140.0
    left_pad.offset_bottom = -12.0
    left_pad.z_index = 301
    left_pad.mouse_filter = Control.MOUSE_FILTER_PASS
    left_pad.add_theme_stylebox_override("panel", panel_style_soft)
    controls_root.add_child(left_pad)

    var lane_title: Label = _make_label("LINE", Vector2(10, 6), 9, Color(0.62, 0.65, 0.68))
    left_pad.add_child(lane_title)
    var lane_up: Button = _make_icon_button(ICON_UP, "UP", Vector2(10, 24), Vector2(70, 88), true)
    ...
    var lane_down: Button = _make_icon_button(ICON_DOWN, "DOWN", Vector2(86, 24), Vector2(70, 88), false)
    ...
    var lean_back: Button = _make_button("LEAN BACK\nWHEELIE", Vector2(168, 16), Vector2(92, 98), false)
    lean_back.name = "LeanBackButton"
    lean_back.add_theme_font_size_override("font_size", 11)
    ...
    var lean_fwd: Button = _make_button("LEAN FWD\nNOSE DOWN", Vector2(266, 16), Vector2(94, 98), false)
    lean_fwd.name = "LeanForwardButton"
    lean_fwd.add_theme_font_size_override("font_size", 10)
    ...
    sensor_pad.offset_left = -138.0
    sensor_pad.offset_right = 138.0
    sensor_pad.offset_top = -70.0
    sensor_pad.offset_bottom = -12.0
    sensor_pad.z_index = 301
    sensor_pad.mouse_filter = Control.MOUSE_FILTER_PASS
    sensor_pad.add_theme_stylebox_override("panel", panel_style_soft)
    controls_root.add_child(sensor_pad)

    tilt_button = _make_button("TILT  ASSIST", Vector2(8, 7), Vector2(112, 44), true)
    tilt_button.add_theme_font_size_override("font_size", 11)
    ...
    var calibrate: Button = _make_button("CAL", Vector2(126, 7), Vector2(64, 44), false)
    calibrate.add_theme_font_size_override("font_size", 11)
    ...
    var invert: Button = _make_button("INV", Vector2(196, 7), Vector2(64, 44), false)
    invert.add_theme_font_size_override("font_size", 11)
    ...
    tilt_status_label = _make_label("", Vector2(8, 48), 8, Color(0.60, 0.63, 0.66))
    tilt_status_label.size = Vector2(250, 10)
    ...
    right_pad.offset_left = -404.0
    right_pad.offset_right = -14.0
    right_pad.offset_top = -140.0
    right_pad.offset_bottom = -12.0
    right_pad.z_index = 301
    right_pad.mouse_filter = Control.MOUSE_FILTER_PASS
    right_pad.add_theme_stylebox_override("panel", panel_style_soft)
    controls_root.add_child(right_pad)

    var brake: Button = _make_button("BRAKE", Vector2(10, 24), Vector2(92, 88), false)
    ...
    var boost: Button = _make_button("BOOST", Vector2(110, 38), Vector2(82, 74), true)
    ...
    var throttle: Button = _make_button("THROTTLE", Vector2(202, 12), Vector2(176, 100), true)
    throttle.name = "ThrottleButton"
    throttle.add_theme_font_size_override("font_size", 16)
...
func _make_bar(pos: Vector2, size_value: Vector2) -> ProgressBar:
    ...
    bar.custom_minimum_size = Vector2(1, 1)
    bar.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
    bar.pivot_offset = Vector2(0, 0)
    bar.scale.y = 0.18
